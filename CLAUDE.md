...
SaaS Starter Template - Next.js 기반의 SaaS 애플리케이션 보일러플레이트입니다.

### 주요 기능

- 사용자 인증 (Google, NAVER OAuth)
- 팀 기반 멀티테넌시
- 구독 결제 시스템 (StepPay)
- 다국어 지원 (한국어/영어)
- 이메일 발송 (Resend)
...
| Authentication | NextAuth v5 (Google, NAVER OAuth) |
...
# 데이터베이스 명령어
pnpm db:setup     # DB 설정 (migrate + seed)
...
- pnpm workspace 사용
- `apps/client`: 메인 애플리케이션
- `packages/ui`: `@repo/ui`로 import
- `packages/tsconfig`: `@repo/tsconfig`로 extend

### App Router 규칙

- 모든 페이지는 `app/[lng]/` 하위에 위치
- 지원 언어: `ko` (한국어), `en` (영어)
- Route Groups:
  - `(auth)`: 로그인/회원가입 페이지
...
- `middleware.ts`에서 인증 체크 및 언어 리다이렉트 처리
- NextAuth v5 사용 (`auth.ts`)
- 세션 기반 인증

### 서버 액션

- `actions/` 디렉토리에 Server Actions 정의
- `'use server'` directive 사용
- 데이터 변경 작업에 사용

### UI 컴포넌트

- shadcn/ui 기반 컴포넌트 사용
- `packages/ui/src/components/` 또는 `apps/client/components/ui/`
...
# Payment (StepPay)
STEPPAY_SECRET_KEY=your-steppay-secret
STEPPAY_WEBHOOK_SECRET=your-webhook-secret
...
Drizzle ORM 사용, 스키마 위치: `apps/client/lib/db/schema.ts`

### Tables

| Table          | Description                                                      |
| -------------- | ---------------------------------------------------------------- |
| `users`        | 사용자 정보 (id, name, email, passwordHash, role, etc.)          |
| `teams`        | 팀 정보 (id, name, createdAt, updatedAt, stripeCustomerId, etc.) |
| `teamMembers`  | 팀-사용자 연결 (userId, teamId, role)                            |
| `activityLogs` | 활동 로그 (userId, teamId, action, timestamp, ipAddress)         |
| `invitations`  | 팀 초대 (teamId, email, role, invitedBy, status)                 |

### Relations

- User → TeamMember → Team (다대다)
- Team → Invitation (일대다)
- User/Team → ActivityLog (일대다)

## Tips for Claude Code

1. **파일 수정 시**: 항상 기존 패턴과 스타일을 따르세요
2. **새 페이지 추가 시**: `app/[lng]/` 하위에 적절한 route group에 생성
3. **DB 스키마 변경 시**: `pnpm db:migrate` 실행 필요
4. **컴포넌트 추가 시**: shadcn/ui 스타일 준수
5. **다국어 텍스트**: `apps/client/lib/i18n/locales/` 번역 파일 수정
